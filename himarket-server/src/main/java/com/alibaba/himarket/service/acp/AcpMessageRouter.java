 ...
 * 前端 ↔ CLI 双向消息路由器。
 *
 * <p>从 AcpWebSocketHandler 中提取消息转发逻辑，职责包括：
 *
 * <ul>
...
public class AcpMessageRouter {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AcpMessageRouter.class);

    /**
     * 订阅 CLI stdout 并转发到前端 WebSocket。
     *
     * <p>从 AcpWebSocketHandler.initSandboxAsync() 中提取的 stdout 订阅逻辑。 当 stdout
     ...
     * 回放初始化期间缓存的待转发消息。
     *
     * <p>从 AcpWebSocketHandler.replayPendingMessages() 迁移。 注意：本方法直接回放消息，不做消息变换。
