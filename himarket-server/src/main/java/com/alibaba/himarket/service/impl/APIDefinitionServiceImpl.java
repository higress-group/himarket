...
public class APIDefinitionServiceImpl implements APIDefinitionService {
    ...
    public APIDefinitionVO createAPIDefinition(CreateAPIDefinitionParam param) {
        ...
        if (param.getEndpoints() != null && !param.getEndpoints().isEmpty()) {
            log.info("Creating {} endpoints for API Definition: {}", param.getEndpoints().size(), apiDefinitionId);
    ...
    public APIDefinitionVO getAPIDefinition(String apiDefinitionId) {
        ...
        APIDefinition apiDefinition = apiDefinitionRepository.findByApiDefinitionId(apiDefinitionId)
                .orElseThrow(() -> new BusinessException(ErrorCode.API_DEFINITION_NOT_FOUND, apiDefinitionId));

        // 转换为 VO（包含 endpoints）
        APIDefinitionVO vo = new APIDefinitionVO().convertFrom(apiDefinition);

        // 查询关联的 Endpoints
        vo.setEndpoints(apiEndpointRepository.findByApiDefinitionIdOrderBySortOrderAsc(apiDefinitionId)
                .stream()
    ...
    public PageResult<APIDefinitionVO> listAPIDefinitions(QueryAPIDefinitionParam param, Pageable pageable) {
        ...
        if (param.getType() != null && param.getStatus() != null) {
            // 同时指定类型和状态
            page = apiDefinitionRepository.findByTypeAndStatus(param.getType(), param.getStatus(), pageable);
        } else if (param.getType() != null) {
            // 仅指定类型
            if (StrUtil.isNotBlank(param.getKeyword())) {
                page = apiDefinitionRepository.findByTypeAndNameContaining(param.getType(), param.getKeyword(), pageable);
            } else {
        ...
        // 转换为 VO
        return new PageResult<APIDefinitionVO>().convertFrom(page, entity -> new APIDefinitionVO().convertFrom(entity));
    }

    @Override
    public APIDefinitionVO updateAPIDefinition(String apiDefinitionId, UpdateAPIDefinitionParam param) {
    ...
    public List<APIEndpointVO> listEndpoints(String apiDefinitionId) {
        ...
        // 查询并转换
        return apiEndpointRepository.findByApiDefinitionIdOrderBySortOrderAsc(apiDefinitionId)
                .stream()
                .map(endpoint -> new APIEndpointVO().convertFrom(endpoint))
    ...
    public APIEndpointVO createEndpoint(String apiDefinitionId, CreateEndpointParam param) {
        ...
        if (endpoint.getSortOrder() == null) {
            int maxSortOrder = apiEndpointRepository.findByApiDefinitionIdOrderBySortOrderAsc(apiDefinitionId)
                    .stream()
    ...
    @Override
    public APIEndpointVO updateEndpoint(String apiDefinitionId, String endpointId, UpdateEndpointParam param) {
        ...
        // 查询 Endpoint
        APIEndpoint endpoint = apiEndpointRepository.findByEndpointIdAndApiDefinitionId(endpointId, apiDefinitionId)
                .orElseThrow(() -> new BusinessException(ErrorCode.ENDPOINT_NOT_FOUND, endpointId));
    ...
    @Override
    public PageResult<APIPublishRecordVO> listPublishRecords(String apiDefinitionId, Pageable pageable) {
        ...
        // 查询发布记录
        Page<APIPublishRecord> page = apiPublishRecordRepository.findByApiDefinitionId(apiDefinitionId, pageable);

        // 转换为 VO
        return new PageResult<APIPublishRecordVO>().convertFrom(
                page,
                record -> new APIPublishRecordVO().convertFrom(record)
        );
    }

    @Override
    public APIPublishRecordVO publishAPI(String apiDefinitionId, PublishAPIParam param) {
        log.info("Publishing API Definition: {} to gateway: {}", apiDefinitionId, param.getGatewayId());
        ...
        // 验证 Gateway 是否存在
        Gateway gateway = gatewayRepository.findByGatewayId(param.getGatewayId())
                .orElseThrow(() -> new BusinessException(ErrorCode.GATEWAY_NOT_FOUND, param.getGatewayId()));

        // 检查是否已发布到该网关
        apiPublishRecordRepository.findByApiDefinitionIdAndGatewayId(apiDefinitionId, param.getGatewayId())
                .ifPresent(record -> {
                    throw new BusinessException(ErrorCode.ALREADY_PUBLISHED, gateway.getGatewayName());
                });
        ...
        // 创建发布历史记录
        createPublishHistory(apiDefinitionId, param.getGatewayId(), recordId, PublishAction.PUBLISH, param.getComment());
    ...
    public void unpublishAPI(String apiDefinitionId, String recordId) {
        ...
        // 查询发布记录
        APIPublishRecord publishRecord = apiPublishRecordRepository.findByRecordId(recordId)
                .orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND, "发布记录", recordId));
    ...
    @Override
    public PageResult<APIPublishHistoryVO> listPublishHistory(String apiDefinitionId, Pageable pageable) {
        ...
        // 查询发布历史（按时间倒序）
        Page<APIPublishHistory> page = apiPublishHistoryRepository
                .findByApiDefinitionIdOrderByCreateAtDesc(apiDefinitionId, pageable);

        // 转换为 VO
        return new PageResult<APIPublishHistoryVO>().convertFrom(
                page,
                history -> new APIPublishHistoryVO().convertFrom(history)
        );
    }
     ...
     * 创建发布历史记录
     */
    private void createPublishHistory(String apiDefinitionId, String gatewayId, String recordId,
                                      PublishAction action, String comment) {
    ...
    private void updatePublishHistoryFailure(String recordId, String errorMessage) {
        apiPublishHistoryRepository.findByApiDefinitionId(recordId)
                .stream()
