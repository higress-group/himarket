...
public abstract class AbstractLlmServiceLegacy implements LlmServiceLegacy {
        ...
        private Flux<ChatAnswerMessage> call(
                ...
                Flux<ChatAnswerMessage> resp = chatContext
                                .getChatClient()
                                ...
                                .flatMap(
                                                chatResponse -> {
                                                        ...
                                                        if (chatResponse.hasToolCalls()) {
                                                                /*
                                                                 * Process LLM response that contains tool/function
                                                                 * calls:
        ...
        private ChatContextLegacy initChatContext(LlmChatRequestLegacy request) {
                ...
                Optional.ofNullable(request.getMcpConfigs()).stream()
                                ...
                                .forEach(
                                                mcpConfig -> {
                                                        McpClientWrapper holder = McpClientFactory.newClient(
                                                        ...
                                                        if (holder != null) {
                                                                ...
                                                                if (tools != null) {
                                                                        tools.forEach(
                                                                                        tool -> {
                                                                                                // Tools meta
                                                                                                McpToolMeta toolMeta = McpToolMeta
                                                                                                                .builder()
                                                                                                                .mcpName(
                                                                                                                                mcpConfig
                                                                                                                                                .getMcpServerName())
                ...
                ChatOptions chatOptions = Optional.of(request.getModelFeature())
                                .map(
                                                feature -> OpenAiChatOptions.builder()
        ...
        private LlmChatRequestLegacy composeRequest(InvokeModelParamLegacy param) {
                ...
                // Web search config
                WebSearchOptions webSearchOptions = (BooleanUtil.isTrue(modelFeature.getWebSearch())
        ...
        private ModelFeature getOrDefaultModelFeature(ProductResult product) {
                ...
                // Get model feature from product or return default values if any field is
                // null/blank
                // Default values: model="qwen-max", maxTokens=5000, temperature=0.9,
                // streaming=true,
                // webSearch=false
                ...
                return Flux.fromIterable(chatResponse.getResults())
                                .flatMap(
                                                generation -> {
                                                        ...
                                                        /*
                                                         * Handle streaming tool calls fragmentation issue with Higress
                                                         * models:
                                                         *
                                                         * Problem: Higress models may split a single tool call into
                                                         * fragments during streaming:
                                                         * Fragment 1: ToolCall(id="123", args="{\"lat\":")
                                                         * Fragment 2: ToolCall(id="123", args=" 39.9}")
                                                         *
                                                         * Solution: Group by id, merge fragments, reconstruct complete
                                                         * tool calls while preserving single ones
                                                         */
                                                        List<AssistantMessage.ToolCall> toolCalls = assistantMessage
                                                                        .getToolCalls();
                                                        Map<String, List<AssistantMessage.ToolCall>> tooCallGroups = toolCalls
                                                                        .stream()
                                                                        .collect(
                                                        ...
                                                        for (var entry : tooCallGroups.entrySet()) {
                                                                List<AssistantMessage.ToolCall> group = entry
                                                                                .getValue();
                                                        ...
                                                        if (hasFragment) {
                                                                ...
                                                                AssistantMessage mergedMessage = AssistantMessage
                                                                                .builder()
                                                                                .content(assistantMessage.getText())
                                                                                .media(assistantMessage.getMedia())
                                                                                .properties(assistantMessage
                                                                                                .getMetadata())
                                                        ...
                                                        Flux<ChatAnswerMessage> toolProcess = Flux.concat(
                                                                        buildToolCallMessages(
                                                                                        mergedToolCalls, usage,
                                                                                        chatContext),
                                                                        executeToolCalls(
                                                                                        usage, chatContext,
                                                                                        responseToExecute));
                ...
                return flux.doOnCancel(
                                ...
                                .onErrorResume(
                                                error -> {
                                                        ...
                                                        return Flux.just(
                                                                        newErrorMessage(chatId, chatError.name(),
                                                                                        error.getMessage()),
                                                                        newChatAnswerMessage(null, null, STOP,
                                                                                        chatContext));
                                                });
        }
                ...
                return Flux.fromIterable(toolCalls)
                                .map(
                                                toolCall -> {
                                                        ...
                                                        ChatAnswerMessage.ToolCall tc = ChatAnswerMessage.ToolCall
                                                                        .builder()
                                                                        .id(toolCall.id())
                                                                        .name(toolName)
                                                                        .type(toolCall.type())
                                        ...
                                        try {
                                                ...
                                                if (lastMessage instanceof ToolResponseMessage toolResponseMessage) {
                                                        ...
                                                        return Flux.fromIterable(toolResponseMessage.getResponses())
                                                                        .map(
                                                                                        response -> newChatAnswerMessage(
                                                                                                        usage,
                                                                                                        ChatAnswerMessage.ToolResponse
                                                                                                                        .builder()
                                                                                                                        .id(response.id())
                                                                                                                        .name(response.name())
                                                                                                                        .responseData(
        ...
        /**
         * Transform OpenAI-compatible ChatMessage objects to SDK-compatible Message
         * objects.
         *
         * @param messages
         * @return
        ...
        /**
         * Builds a ChatClient instance according to the specified protocol (e.g.,
         * OpenAI, etc.) based
