...
public abstract class AbstractLlmService implements LlmService {
     ...
     * @return
     */
    private Flux<ChatAnswerMessage> call(LlmChatRequest request, Consumer<LlmInvokeResult> resultHandler) {
...
//                        );
     ...
     * @return
     */
    private Flux<ChatAnswerMessage> streamToolCalls(ChatContext chatContext, ChatResponse chatResponse,
        ...
        return Flux.fromIterable(chatResponse.getResults())
                .flatMap(generation -> {
                    ...
                    Map<String, List<AssistantMessage.ToolCall>> tooCallGroups = toolCalls.stream()
     ...
     * @return
     */
    private Flux<ChatAnswerMessage> streamAnswer(ChatResponse chatResponse, ChatContext chatContext) {
        Usage usage = chatResponse.getMetadata().getUsage();
        return Flux.fromIterable(chatResponse.getResults())
                .map(generation -> {
                    AssistantMessage assistantMessage = generation.getOutput();
     ...
     * @return
     */
    private Flux<ChatAnswerMessage> continueNextCall(ChatContext chatContext, Consumer<LlmInvokeResult> resultHandler) {
     ...
     * @return
     */
    private Flux<ChatAnswerMessage> buildToolCallMessages(List<AssistantMessage.ToolCall> toolCalls, Usage usage,
     ...
     * @return
     */
    private Flux<ChatAnswerMessage> executeToolCalls(Usage usage, ChatContext chatContext, ChatResponse chatResponse) {
        return Flux.defer(() -> {
     ...
     * @return
     */
    protected abstract URL getUrl(ModelConfigResult modelConfig, Map<String, String> queryParams, List<String> gatewayIps);
