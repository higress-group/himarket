...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  -n, --non-interactive  跳过交互式提示，使用 .env / 默认值"
...
msg() {
    ...
    case "${key}" in
        ...
        install.confirm_save)
            [[ "$lang" == "zh" ]] && text="是否保存配置到 .env? [Y/n]" || text="Save config to .env? [Y/n]" ;;
        install.cancelled)
        ...
        prompt.required)
            [[ "$lang" == "zh" ]] && text="错误: %s 是必需的（非交互模式请通过环境变量或 .env 设置）" || text="Error: %s is required (set via env var or .env in non-interactive mode)" ;;
        ...
        install.ai_model_existing_title)
            [[ "$lang" == "zh" ]] && text="检测到 .env 中已配置的 AI 模型:" || text="Existing AI models from .env:" ;;
