...
# 优先尝试 LoadBalancer IP，失败则降级为 ClusterIP
NACOS_HOST=$(kubectl get svc nacos -n "${NS}" -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || echo "")

if [[ -z "$NACOS_HOST" ]]; then
  log "未检测到 LoadBalancer IP，使用 ClusterIP 模式"
  NACOS_HOST="nacos.${NS}.svc.cluster.local"
fi

# Nacos 3.0 API 路径（不需要 /nacos 前缀）
