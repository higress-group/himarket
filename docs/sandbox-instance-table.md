...
| `sandbox_name` | varchar(64), 唯一 | 是 | - | 实例名称，全局唯一 |
...
| `kube_config` | text | 否 | NULL | 完整的 KubeConfig 文本 |
...
| `last_checked_at` | datetime | 否 | NULL | 上次健康检查时间 |
...
| `uk_sandbox_name` | 唯一索引 | `sandbox_name` | 实例名称全局唯一 |
...
CREATE TABLE IF NOT EXISTS `sandbox_instance` (
    ...
    `cluster_attribute` json DEFAULT NULL,
    ...
    UNIQUE KEY `uk_sandbox_name` (`sandbox_name`),
